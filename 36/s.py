...
def run():
    ...
    robot_locs = [(x, y) for y, row in enumerate(m) for x, val in enumerate(row) if m[y][x] == '@']
    ...
    print('Final answer:', solve_full_search(m, robot_locs, key_locs, door_locs, 0, 9999999999999999, [None], {}))
...
def solve_full_search(m, robot_locs, key_locs, door_locs, dist_here, min_so_far, curr_key_order, cache):
    if len(key_locs) == 0:
        return dist_here
    ...
    avail_keys = find_keys(m, robot_locs)   # They'll come back sorted by distance already.

    min_on_this_path = 999999999999999

    for i, (robot_index, k, kx, ky, key_dist) in enumerate(avail_keys):
        ...
        orig_x, orig_y = robot_locs[robot_index]
        robot_locs[robot_index] = (kx, ky)
        val_here = solve_full_search(m, robot_locs, key_locs, door_locs, dist_here + key_dist, min_so_far, curr_key_order, cache)
        robot_locs[robot_index] = (orig_x, orig_y)
        key_locs.add((kx, ky))
        curr_key_order.pop()
...
def find_keys(m, robot_locs):
    q = deque()
    visited = set()

    for i, (x, y) in enumerate(robot_locs):
        q.append((i, x, y, 0))
        visited.add((x, y))
    ...
    while len(q) > 0:
        ...
        robot_index, x, y, dist = q.popleft()

        if m[y][x-1] in CAN_VISIT and (x-1, y) not in visited:
            visited.add((x-1, y))
            if m[y][x-1] in KEYS:
                k = m[y][x-1]
                avail_keys.append((robot_index, k, x-1, y, dist+1))
            else:
                q.append((robot_index, x-1, y, dist+1))
        ...
        if m[y-1][x] in CAN_VISIT and (x, y-1) not in visited:
            ...
            if m[y-1][x] in KEYS:
                k = m[y-1][x]
                avail_keys.append((robot_index, k, x, y-1, dist+1))
            else:
                q.append((robot_index, x, y-1, dist+1))
