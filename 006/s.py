...
paths = [l.split(',') for l in input_.splitlines()]
...
def build_set(path):
    ...
    steps_map = {}   # map coordinate to the number of steps taken to arrive there

    for cmd in path:
        dist = int(cmd[1:])

        if cmd.startswith('R'):
            for x in range(1, 1+dist):
                s.add((curr_x + x, curr_y))
                steps_map[curr_x + x, curr_y] = steps + 1
                steps += 1
            curr_x += dist

        elif cmd.startswith('U'):
            for y in range(1, 1+dist):
                s.add((curr_x, curr_y + y))
                steps_map[curr_x, curr_y + y] = steps + 1
                steps += 1
            curr_y += dist

        elif cmd.startswith('L'):
            for x in range(-1, -1-dist, -1):
                s.add((curr_x + x, curr_y))
                steps_map[curr_x + x, curr_y] = steps + 1
                steps += 1
            curr_x -= dist
...
a, a_steps = build_set(paths[0])
b, b_steps = build_set(paths[1])

intersection = a & b

signal_delays = [a_steps[coord] + b_steps[coord] for coord in intersection]

print(min(signal_delays))
